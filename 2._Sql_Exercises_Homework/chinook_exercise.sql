...
SELECT artist.Name
FROM artist
WHERE (Name REGEXP 'Jack' OR Name REGEXP 'John')
...
SELECT SUM(Total) AS monetaryAmount
FROM invoice
...
SELECT DISTINCT Album.AlbumId, Title, ArtistId
...
IF NULL(Fax, 'S/he has no fax') AS fax
ROM customer
WHERE Country = 'USA'
